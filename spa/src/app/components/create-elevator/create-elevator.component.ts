...
import { ElevatorDTO, ElevatorService } from 'src/app/services/elevator.service';
...
export class CreateElevatorComponent {
  ...
  constructor(private fb: FormBuilder,
      ...
      buildingService.getBuildings().subscribe((buildingsList: BuildingDTO[]) => {
      this.buildings = buildingsList;
      });


      this.elevatorForm.get('selectedBuildingCode')?.valueChanges.subscribe((selectedBuildingCode) => {
          if (selectedBuildingCode !== null && selectedBuildingCode !== undefined) {
              this.floorService.getFloors(selectedBuildingCode).subscribe((floorsList: Floor[]) => {
    ...
    submitElevatorForm() {
        ...
        if (this.elevatorForm.valid) {
            ...
            const selectedBuildingCode = this.elevatorForm.get('selectedBuildingCode')?.value;
            const selectedFloors = this.elevatorForm.get('selectedFloors')?.value;
