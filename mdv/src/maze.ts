...
type GroundT = {
    ...
    segments: { width: number, height: 1, depth: 1 },
    primaryColor: string,
    maps: {
        color: { url: string },
...
type WallT = {
    segments: { width: number, height: 1 },
    primaryColor: string,
    maps: {
        color: { url: string },
        ao: { url: string, intensity: number }
...
export default class Maze extends THREE.Group {
    get url() { return this.parameters.url }
    get designCredits() { return this.parameters.designCredits }
    get texturesCredits() { return this.parameters.texturesCredits }
    get helpersColor() { return this.parameters.helpersColor }
    ...
    cornerCollision(indices: number[], offsets: number[], orientation: number, position: THREE.Vector3, delta: Position, radius: number, name: string) {
    ...
    wallCollision(indices: number[], offsets: number[], orientation: number, position: THREE.Vector3, delta: Position, radius: number, name: string) {
        const row = indices[0] + offsets[0];
        const column = indices[1] + offsets[1];
        if (this.map[row][column] == 2 - orientation || this.map[row][column] == 3) {
            if (orientation != 0) {
    ...
    wallAndCornerCollision(indices: number[], offsets: number[], orientation: number, obb: OBB, name: string) {
        const row = indices[0] + offsets[0];
        const column = indices[1] + offsets[1];
        if (this.map[row][column] == 2 - orientation || this.map[row][column] == 3) {
    ...
    // Detect collisions
    collision(method: string, position: THREE.Vector3, halfSize: number, direction: number) {
