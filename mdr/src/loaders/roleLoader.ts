...
export default async (roles: string[]) => {
    ...
    const activeRoles = (await svc.getRoles()).getOrThrow().map(r => constantCase(r.name))
    ...
    if (toAdd.length > 0) {
        ...
        toAdd.forEach(async r => {
            ...
            if (result.isFailure) {
                logger.error(`Failed to create role: ${r}. Reason: ${result.errorValue()}`)
            } else {
    ...
    if (toDisable.length > 0) {
        ...
        toDisable.forEach(async r => {
            ...
            if (result.isFailure) {
                logger.error(`Failed to disable role: ${r}. Reason: ${result.errorValue()}`)
            } else {
