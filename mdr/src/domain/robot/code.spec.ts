...
describe('Serial Number', () => {
    it('cannot be longer than 30 characters', () => {
        ...
        codes.forEach((b) => assert.isNotOk(Code.create(b).isSuccess))

        // less than 50 chars
        assert.isOk(Code.create('3100').isSuccess)
    })

    it('must be alphanumeric', () => {
        let code = Code.create('XSW43161ELH')
        assert.isOk(code.isSuccess)

        code = Code.create('16280025')
        assert.isOk(code.isSuccess)

        code = Code.create('C3NE1001')
        assert.isOk(code.isSuccess)

        code = Code.create('fad99132#41941$18013@@aa{12:2131}')
        assert.isNotOk(code.isSuccess)

        code = Code.create('_daida?ad9123:?=adhival')
        assert.isNotOk(code.isSuccess)
    })
})
