...
describe('Passage integration', () => {
    ...
    let code1:BuildingCode,code2: BuildingCode, maxFloorDimensionsRes: MaxFloorDimensions
    let building1: Building, building2: Building

    beforeEach(() => {
        stubCreate(BuildingCode)
        stubCreate(FloorNumber)
        sinon.stub(MaxFloorDimensions, 'create').returns(Result.ok({width:10,length:10} as MaxFloorDimensions))
