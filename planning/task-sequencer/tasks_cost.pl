:- module('task_cost',[create_task_costs/1]).
...
generate_tasks(List):-
    findall(_,calculate_and_assert_task_costs(List),_).

calculate_and_assert_task_costs([]).
calculate_and_assert_task_costs(List) :-
    member(Task,List)
